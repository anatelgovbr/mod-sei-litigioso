...
if ($numRegistros > 0) {
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
        ...
        if (!$bolSelecionar) {
            ...
            //Ação Consultar
            $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'consultar&acao_origem=' . $_GET['acao'] . '&id_md_lit_situacao=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Situação de Lançamento" alt="Consultar Situação de Lançamento" class="infraImg" /></a>&nbsp;';

            if ($arrObjMdLitSituacaoLancamentoDTO[$i]->getStrStaOrigem() == MdLitSituacaoLancamentoRN::$STA_ORIGEM_MANUAL && $bolRegistroAtivo) {
                //Ação Alterar
                $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'alterar&acao_origem=' . $_GET['acao'] . '&id_md_lit_situacao=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Situação de Lançamento" alt="Alterar  Situação de Lançamento" class="infraImg" /></a>&nbsp;';
                ...
                //Ação Alterar
                $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'alterar&acao_origem=' . $_GET['acao'] . '&id_md_lit_situacao=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="modulos/litigioso/imagens/integracao.png" style="height: 0.9em;" title="Alterar Situação de Lançamento" alt="Alterar Situação de Lançamento" class="infraImg" /></a>&nbsp;';
            }

            if ($bolRegistroAtivo) {
                //Ação Desativar
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="desativar(\'' . $strId . '\',\'' . $strNomeParametro . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/desativar.gif" title="Desativar Situação de Lançamento" alt="Desativar Situação de Lançamento" class="infraImg" /></a>&nbsp;';
            } else {
                //Ação Reativar
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="reativar(\'' . $strId . '\',\'' . $strNomeParametro . '\', \''.SessaoSEI::getInstance()->assinarLink($strUrl . "reativar&acao_origem=" . $_GET["acao"].PaginaSEI::getInstance()->montarAncora($strId)).'\')" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/reativar.gif" title="Reativar Situação de Lançamento" alt="Reativar Situação de Lançamento" class="infraImg" /></a>&nbsp;';
            }

            if($arrObjMdLitSituacaoLancamentoDTO[$i]->getStrStaOrigem() == MdLitSituacaoLancamentoRN::$STA_ORIGEM_MANUAL){
                //Ação Excluir
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="excluir(\'' . $strId . '\',\'' . $strNomeParametro . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Situação de Lançamento" alt="Excluir Situação de Lançamento" class="infraImg" /></a>&nbsp;';
            }
