    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if ($bolAcaoDesativar && $arrObjDispositivoNormativoLitigiosoDTO[$i]->getStrSinAtivo() == 'S' && $arrObjDispositivoNormativoLitigiosoDTO[$i]->getStrSinRevogado() != 'S') {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoDesativar(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/desativar.svg" title="Desativar Dispositivo Normativo" alt="Desativar Dispositivo Normativo" class="infraImg" /></a>&nbsp;';
            } else if ($bolAcaoReativar && $arrObjDispositivoNormativoLitigiosoDTO[$i]->getStrSinRevogado() != 'S') {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoReativar(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/reativar.svg" title="Reativar Dispositivo Normativo" alt="Reativar Dispositivo Normativo" class="infraImg" /></a>&nbsp;';
            }

            if ($bolAcaoExcluir && $arrObjDispositivoNormativoLitigiosoDTO[$i]->getStrSinRevogado() != 'S') {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoExcluir(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/excluir.svg" title="Excluir Dispositivo Normativo" alt="Excluir Dispositivo Normativo" class="infraImg" /></a>&nbsp;';
            }
