    ...
    try {
        ...
        if ($numRegistros > 0) {
            ...
            for ($i = 0; $i < $numRegistros; $i++) {
                ...
                if ($bolAcaoConsultar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_tipo_decisao_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_tipo_decisao_litigioso=' . $arrObjTipoDecisaoLitigiosoDTO[$i]->getNumIdTipoDecisaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="imagens/consultar.gif" title="Consultar Tipo de Decisão" alt="Consultar Tipo de Decisão" class="infraImg" /></a>&nbsp;';
                }

                if ($bolAcaoAlterar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_tipo_decisao_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_tipo_decisao_litigioso=' . $arrObjTipoDecisaoLitigiosoDTO[$i]->getNumIdTipoDecisaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Tipo de Decisão" alt="Alterar Tipo de Decisão" class="infraImg" /></a>&nbsp;';
                }
...
?>
    ...
    #lblTipoDecisao  {position:absolute;left:0%;top:0%;}
    #txtTipoDecisao {position:absolute;left:0%;top:40%;width:20%;}
