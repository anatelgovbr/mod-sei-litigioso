    ...
    try {
        ...
        if ($numRegistros > 0) {
            ...
            for ($i = 0; $i < $numRegistros; $i++) {
                ...
                if ($bolAcaoConsultar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_especie_decisao_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_especie_decisao_litigioso=' . $arrObjEspecieLitigiosoDTO[$i]->getNumIdEspecieLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Espécie de Decisão" alt="Consultar Espécie de Decisão" class="infraImg" /></a>&nbsp;';
                }

                if ($bolAcaoAlterar) {

                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_especie_decisao_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_especie_decisao_litigioso=' . $arrObjEspecieLitigiosoDTO[$i]->getNumIdEspecieLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Espécie de Decisão" alt="Alterar Espécie de Decisão" class="infraImg" /></a>&nbsp;';
                }
                ...
                if ($bolAcaoDesativar && $arrObjEspecieLitigiosoDTO[$i]->getStrSinAtivo() == 'S') {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoDesativar(\'' . $strId . '\', \'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/desativar.gif" title="Desativar Espécie de Decisão" alt="Desativar Espécie de Decisão" class="infraImg" /></a>&nbsp;';
                } else if ($bolAcaoDesativar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoReativar(\'' . $strId . '\', \'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/reativar.gif" title="Reativar Espécie de Decisão" alt="Reativar Espécie de Decisão" class="infraImg" /></a>&nbsp;';
                }

                if ($bolAcaoExcluir) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoExcluir(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Espécie de Decisão" alt="Excluir Espécie de Decisão" class="infraImg" /></a>&nbsp;';
                }
...
?>
    <form id="frmEspecieLitigiosoLista" method="post"
        ...
        <div style="height:4.5em;" class="infraAreaDados" id="divInfraAreaDados">
            <label id="lblEspecieDecisao" for="txtEspecieDecisao" accesskey="S" class="infraLabelOpcional">Espécie de
                Decisão:</label>
            <input type="text" id="txtEspecieDecisao" name="txtEspecieDecisao" class="infraText"
