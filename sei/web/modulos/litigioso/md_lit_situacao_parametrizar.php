        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            if($_GET['acao'] != 'md_lit_situacao_visualizar_parametrizar' ){
                $strImagem = '<a onclick="moverAcima(this)"><img src="'
                    . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/mover_acima.svg" style="margin: 2px -7px 12px 2px;" /></a>';

                $strImagem .= '<a onclick="moverAbaixo(this)" "><img src="'
                    . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/mover_abaixo.svg" /> </a>';
            }
            ...
            if ($bolAcaoConsultar) {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_situacao_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_tipo_processo_litigioso=' . $_GET['id_tipo_processo_litigioso'] . '&id_situacao_litigioso=' . $arrObjSituacaoLitigiosoDTO[$i]->getNumIdSituacaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/consultar.svg" title="Consultar Situação" alt="Consultar Situação" class="infraImg" /></a>&nbsp;';
            }

            if ($bolAcaoAlterar) {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_situacao_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_tipo_processo_litigioso=' . $_GET['id_tipo_processo_litigioso'] . '&id_situacao_litigioso=' . $arrObjSituacaoLitigiosoDTO[$i]->getNumIdSituacaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/alterar.svg" title="Alterar Situação" alt="Alterar Situação" class="infraImg" /></a>&nbsp;';
            }
            ...
            if ($bolAcaoDesativar && $arrObjSituacaoLitigiosoDTO[$i]->getStrSinAtivo() == 'S') {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/desativar.svg" title="Desativar Situação" alt="Desativar Situação" class="infraImg"  id="imgDesativar_' . $strId . '" onclick="acaoDesativar(this,\'' . $strId . '\',1);"/></a>&nbsp;';
            } elseif($bolAcaoReativar) {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/reativar.svg" title="Reativar Situação" alt="Reativar Situação" class="infraImg" id="imgReativar_' . $strId . '" onclick="acaoReativar(this,\'' . $strId . '\',1);"/></a>&nbsp;';
            }


            if ($bolAcaoExcluir) {
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoExcluir(this,\'' . $strId . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/excluir.svg" title="Excluir Situação" alt="Excluir Situação" class="infraImg" /></a>&nbsp;';
            }
