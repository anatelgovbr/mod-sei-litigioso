    ...
    try {
        ...
        if ($numRegistros > 0) {
            ...
            for ($i = 0; $i < $numRegistros; $i++) {
                ...
                if ($bolAcaoConsultar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_obrigacao_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_obrigacao_litigioso=' . $arrObjObrigacaoLitigiosoDTO[$i]->getNumIdObrigacaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Obrigação" alt="Consultar Obrigação" class="infraImg" /></a>&nbsp;';
                }

                if ($bolAcaoAlterar) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_obrigacao_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_obrigacao_litigioso=' . $arrObjObrigacaoLitigiosoDTO[$i]->getNumIdObrigacaoLitigioso())) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Obrigação" alt="Alterar Obrigação" class="infraImg" /></a>&nbsp;';
                }
                ...
                if ($bolAcaoDesativar && $arrObjObrigacaoLitigiosoDTO[$i]->getStrSinAtivo() == 'S') {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoDesativar(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/desativar.gif" title="Desativar Obrigação" alt="Desativar ' . $funcionalidade . '" class="infraImg" /></a>&nbsp;';
                }

                if ($bolAcaoReativar && $arrObjObrigacaoLitigiosoDTO[$i]->getStrSinAtivo() == 'N') {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoReativar(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/reativar.gif" title="Reativar Obrigação" alt="Reativar ' . $funcionalidade . '" class="infraImg" /></a>&nbsp;';
                }


                if ($bolAcaoExcluir) {
                    $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="acaoExcluir(\'' . $strId . '\',\'' . $strDescricao . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Obrigação" alt="Excluir ' . $funcionalidade . '" class="infraImg" /></a>&nbsp;';
                }
...
?>
    ...
    #lblNomeObrigacaoPesquisa {position:absolute;left:0%;top:0%;}
    #txtNomeObrigacaoPesquisa {position:absolute;left:0%;top:40%;width:40%;}
...
?>
    <form id="frmObrigacaoLista" method="post"
        ...
        <input type="text" id="txtNomeObrigacaoPesquisa" name="txtNomeObrigacaoPesquisa"
               value="<?= $strNomeObrigacaoPesquisa ?>" class="infraText"
