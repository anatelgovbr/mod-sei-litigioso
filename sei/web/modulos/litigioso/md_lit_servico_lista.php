...
if ($numRegistros > 0) {
    ...
    for ($i = 0; $i < $numRegistros; $i++) {
        ...
        if (!$bolSelecionar) {
            ...
            //A??o Consultar
            $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'consultar&acao_origem=' . $_GET['acao'] . '&id_servico=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/consultar.gif" title="Consultar Serviço" alt="Consultar Serviço" class="infraImg" /></a>&nbsp;';

            if ($arrObjMdLitServicoDTO[$i]->getStrStaOrigem() == MdLitServicoRN::$STA_ORIGEM_MANUAL && $bolRegistroAtivo) {
                //A??o Alterar
                $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'alterar&acao_origem=' . $_GET['acao'] . '&id_servico=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/alterar.gif" title="Alterar Serviço" alt="Alterar Serviço" class="infraImg" /></a>&nbsp;';
            }elseif ($arrObjMdLitServicoDTO[$i]->getStrStaOrigem() == MdLitServicoRN::$STA_ORIGEM_INTEGRACAO && $bolRegistroAtivo) {
                //A??o Alterar
                $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink($strUrl . 'alterar&acao_origem=' . $_GET['acao'] . '&id_servico=' . $strId) . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="modulos/litigioso/imagens/integracao.png" style="height: 0.9em;" title="Alterar Serviço" alt="Alterar Integração" class="infraImg" /></a>&nbsp;';
            }

            if ($bolRegistroAtivo) {
                //A??o Desativar
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="desativar(\'' . $strId . '\',\'' . $strDescricaoParametro . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/desativar.gif" title="Desativar Serviço" alt="Desativar Serviço" class="infraImg" /></a>&nbsp;';
            } else {
                //A??o Reativar
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="reativar(\'' . $strId . '\',\'' . $strDescricaoParametro . '\', \''.SessaoSEI::getInstance()->assinarLink($strUrl . "reativar&acao_origem=" . $_GET["acao"].PaginaSEI::getInstance()->montarAncora($strId)).'\')" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/reativar.gif" title="Reativar Serviço" alt="Reativar Serviço" class="infraImg" /></a>&nbsp;';
            }

            if($arrObjMdLitServicoDTO[$i]->getStrStaOrigem() == MdLitServicoRN::$STA_ORIGEM_MANUAL){
                //A??o Excluir
                $strResultado .= '<a href="' . PaginaSEI::getInstance()->montarAncora($strId) . '" onclick="excluir(\'' . $strId . '\',\'' . $strDescricaoParametro . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioImagensGlobal() . '/excluir.gif" title="Excluir Serviço" alt="Excluir Serviço" class="infraImg" /></a>&nbsp;';
            }
