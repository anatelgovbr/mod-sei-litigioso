...
function normalizarValorDecisaoCadastro($valor)
{
    if ($valor === null || $valor === '') {
        return '0.00';
    }
...
function consultarIdLancamentoDecisaoCadastro($idDecisao)
{
    if (!$idDecisao || !is_numeric($idDecisao)) {
        return null;
    }
...
//    InfraDebug::getInstance()->limpar();
    ...
    $strTbDecisaoAntigo = '';
    $arrLancamentosAlteradosDecisao = array();
    ...
    switch ($_GET['acao']) {
        case 'md_lit_decisao_cadastrar':
            ...
            if (count($_POST)) {
                ...
                $strTbDecisaoAntigo = $hdnTbDecisaoAntigo;
                $bolCadastro = true;
                if ($hdnTbDecisaoAntigo != '') {
                    ...
                    $arrDecisoesAntigasPorId = array();
                    foreach ($arrTbDecisaoAntigo as $arrDecisaoAntiga) {
                        if (isset($arrDecisaoAntiga[0]) && $arrDecisaoAntiga[0] !== '') {
                            $arrDecisoesAntigasPorId[$arrDecisaoAntiga[0]] = $arrDecisaoAntiga;
                        }
                    }

                    foreach ($arrTbDecisaoNovo as $arrDecisaoNova) {
                        $idComparacao = isset($arrDecisaoNova[0]) ? $arrDecisaoNova[0] : '';
                        if (strpos($idComparacao, 'novo_') === 0 && isset($arrDecisaoNova[20]) && $arrDecisaoNova[20] !== '') {
                            $idComparacao = $arrDecisaoNova[20];
                        }

                        if (!isset($arrDecisoesAntigasPorId[$idComparacao])) {
                            continue;
                        }

                        $arrDecisaoAntiga = $arrDecisoesAntigasPorId[$idComparacao];
                        $valorAntigo = normalizarValorDecisaoCadastro($arrDecisaoAntiga[4]);
                        $valorNovo = normalizarValorDecisaoCadastro($arrDecisaoNova[4]);

                        if (bccomp($valorAntigo, $valorNovo, 2) == 0) {
                            continue;
                        }

                        $idLancamento = consultarIdLancamentoDecisaoCadastro($idComparacao);
                        if (!$idLancamento) {
                            $idLancamento = isset($arrDecisaoNova[19]) ? $arrDecisaoNova[19] : '';
                        }
                        if ($idLancamento === '' || $idLancamento === 'null') {
                            $idLancamento = isset($arrDecisaoAntiga[19]) ? $arrDecisaoAntiga[19] : '';
                        }

                        if ($idLancamento !== '' && $idLancamento !== 'null') {
                            $arrLancamentosAlteradosDecisao[$idLancamento] = $idLancamento;
                        }
                    }
                    ...
                    if (!$bolHouveMudanca) {
                        for ($i = 0; $i < count($arrTbDecisaoAntigo); $i++) {
                            if ($arrTbDecisaoAntigo[$i][1] != $arrTbDecisaoNovo[$i][1] ||
                                ...
                                $arrTbDecisaoAntigo[$i][19] != $arrTbDecisaoNovo[$i][19]
                                || $arrTbDecisaoAntigo[$i][20] != $arrTbDecisaoNovo[$i][20]
                            )
                                $bolHouveMudanca = true;
                        }
                    }
...
$strTbCadastroDecisao .= "<input type='hidden' name='decisao[idDispositivoNormativo_{$idLinha}][id_decisao_origem]' value='' /> ";
        $strTbCadastroDecisao .= "<input type='hidden' name='decisao[idDispositivoNormativo_{$idLinha}][id_lancamento]' value='null' /> ";
