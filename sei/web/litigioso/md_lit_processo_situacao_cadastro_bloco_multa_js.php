<script type="text/javascript">
    var prazoDiasSugVenc = parseInt('<?= (int) $numPrazoDiasSugVenc ?>', 10);
    var prazoDiasCritVencMin = parseInt('<?= (int) $numPrazoDiasCritVencMin ?>', 10);
    ...
    function consultarExtratoMulta() {
        ...
        //se o tipo da multa for por indicação de valor e nao houver ignora as validações e exibião do fieldset de multa
        ...
        var manterRetificacaoDupla = document.getElementById('hdnRetificacaoDupla').value == 'true';
        document.getElementById('btnRetificarLancamento').style.display = 'none';
        document.getElementById('btnSalvarLancamentos').style.display = manterRetificacaoDupla ? '' : 'none';
        ...
        document.getElementById('msgRetificacaoDupla').style.display = manterRetificacaoDupla ? '' : 'none';
        document.getElementById('detalhesLancamento').style.display = manterRetificacaoDupla ? 'none' : '';
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if ($(result).find('dados').length > 0) {
                    ...
                    var hdnLancamentoFracionado = $(result).find('hdnLancamentoFracionado').text();
                    if (manterRetificacaoDupla && hdnLancamentoFracionado != 'true') {
                        hdnLancamentoFracionado = 'true';
                    }
                    document.getElementById('hdnLancamentoFracionado').value = hdnLancamentoFracionado;
                    document.getElementById('hdnIdLancamentoInicial').value = $(result).find('hdnIdLancamentoInicial').text();
                    ...
                    atualizarTextoOpcaoCreditoProcesso(selCreditosProcesso, $(result).find('multaAplicada').text());
                    ...
                    if (sinExibeCancelamento == 'N' && creditoNaoLancado != '0.00' && isNovoLancamento == 'S' && selCreditosProcesso == '') {
                        ...
                        if (!optionExiste('', document.getElementById('selCreditosProcesso'))) {
                            var qtde = retornarProximoIndiceMajorado(document.getElementById('selCreditosProcesso'));
                    ...
                    if (document.getElementById('hdnRetificacaoDupla').value == 'true') {
                        verificarExibicaoDivLancamentoFracionado();
                    }
    ...
    function retificarLancamento(element){
        if (!podeAcionarLancamento()) {
            return;
        }
        abrirModalJustificativaLancamento(element);
    }

    function podeAcionarLancamento(){
        ...
        //Para atender essa regra, identificamos se houve alteração no cadastro da decisão a partir do valor do crédito não lançado
        if (document.getElementById('hdnLancamentoFracionado').value == 'false' && '<?=$idUltimoLancamento?>' != document.getElementById('selCreditosProcesso').value && document.getElementById('lblVlCreditoNaoLancado').innerText != 'R$ 0,00') {
    ...
    function salvarLancamentosDireto(element){
        if (!podeAcionarLancamento()) {
            return;
        }
        if (!validarCamposObrigatoriosGestaoMulta()) {
            return;
        }

        document.getElementById('hdnIdMdLitFuncionalidade').value = '<?= MdLitIntegracaoRN::$ARRECADACAO_RETIFICAR_LANCAMENTO ?>';
        document.getElementById('hdnJustificativaLancamento').value = '';
        document.getElementById('sbmCadastrarSituacao').click();
    }
    ...
    function validarCamposObrigatoriosGestaoMulta() {
        var dadosComplementares = '<?php echo $objMdLitTipoControleDTO->getStrSinParamModalComplInteressado()?>';
        ...
        if (document.getElementById('txtDtIntimacaoAplMulta').value && validarDataVencimentoLancamento()) {
            alert(montarMensagemDataVencimentoInvalida());
            return false;
        }
    ...
    function selecionarLancamentoAlteradoDecisao() {
        var idLancamento = document.getElementById('hdnLancamentoAlteradoDecisao').value;
        var selCreditosProcesso = document.getElementById('selCreditosProcesso');

        if (!selCreditosProcesso) {
            return;
        }

        if (!idLancamento || !optionExiste(idLancamento, selCreditosProcesso)) {
            selCreditosProcesso.removeAttribute('disabled');
            document.getElementById('hdnCreditosProcesso').value = selCreditosProcesso.value;
            return;
        }

        selCreditosProcesso.value = idLancamento;
        document.getElementById('hdnCreditosProcesso').value = idLancamento;
        selCreditosProcesso.setAttribute('disabled', 'disabled');
    }
    ...
    function retornarProximoIndiceMajorado(element) {
        var maiorIndice = 0;
        var indicesComRaizPropria = {};

        Array.from(element.options).forEach(option => {
            var match = option.text.match(/^Majorado\s+(\d+)/);
            if (match) {
                var indice = parseInt(match[1], 10);
                maiorIndice = Math.max(maiorIndice, indice);
                if (option.text.indexOf('(Fracionado') === -1) {
                    indicesComRaizPropria[indice] = true;
                }
            }
        });

        // Se o maior indice encontrado so tem fracionados visiveis (raiz cancelada/
        // ausente da combo), o novo majorado vai ser adotado como a raiz desse mesmo
        // grupo ao salvar (MdLitLancamentoRN::adotarFracionadosMajoradoOrfaos); nao
        // deve incrementar o indice nessa previa.
        if (maiorIndice > 0 && !indicesComRaizPropria[maiorIndice]) {
            return maiorIndice;
        }
    ...
    function verificarCondicionaisMulta() {
        ...
        if (document.getElementById('btnSalvarLancamentos').style.display == 'none') {
        if(
    ...
    function montarMensagemDataVencimentoInvalida() {
        if (prazoDiasCritVencMin == 0) {
            return 'A Data de Vencimento inserida não é válida, somente é permitida Data de Vencimento igual ou posterior à Data de Intimação da Decisão que aplicou a Multa.';
        }
        return 'A Data de Vencimento inserida não é válida, somente é permitida Data de Vencimento com diferença mínima de ' + prazoDiasCritVencMin + ' dias após a Data de Intimação da Decisão que aplicou a Multa.';
    }

    function validarDataVencimentoLancamento() {
        var dtaIntimacao = document.getElementById('txtDtIntimacaoAplMulta').value;
        var dtaPermitida = infraCalcularDataDias(dtaIntimacao, prazoDiasCritVencMin).split("/");
    ...
    function verificarExibicaoDivLancamentoFracionado()
    {
        var exibirRetificacaoDupla = document.getElementById('hdnRetificacaoDupla').value == 'true';
        document.getElementById('detalhesLancamento').style.display = exibirRetificacaoDupla ? 'none' : '';
        document.getElementById('msgRetificacaoDupla').style.display = exibirRetificacaoDupla ? '' : 'none';
        document.getElementById('btnSalvarLancamentos').style.display = exibirRetificacaoDupla ? '' : 'none';

        if (exibirRetificacaoDupla) {
            ocultarBotoesLancamentoParaSalvarLancamentos();
        }
    }

    function ocultarBotoesLancamentoParaSalvarLancamentos(doc)
    {
        doc = doc || document;
        var botoes = [
        ...
        for (var i = 0; i < botoes.length; i++) {
            var botao = doc.getElementById(botoes[i]);
            if (botao) {
                mostrarEsconderElemento(botao, 'none');
            }
        }
    }
    ...
    function verificarNecessidadeAlteracaoDataValidade(){
        ...
        novaDtaVencimentoApartirIntimacao = infraCalcularDataDias(novaDtaVencimentoApartirIntimacao, prazoDiasCritVencMin);
    ...
    function calcularData() {
        ...
        if (
            ...
            //calcula a ultima data da decisão cadastrada e adiciona o prazo parametrizado
            var dtaVencimento = infraCalcularDataDias(dtaDecisaoAplicacaoMulta, prazoDiasSugVenc);
    ...
    function atualizarTextoOpcaoCreditoProcesso(idLancamento, valorFormatado) {
        if (!idLancamento || idLancamento === 'null') {
            return;
        }

        var select = document.getElementById('selCreditosProcesso');
        if (!select) {
            return;
        }

        var option = select.querySelector('option[value="' + idLancamento + '"]');
        if (!option) {
            return;
        }

        var textoAtual = option.text || '';

        if (/\(R\$\s*[^\)]+\)\s*$/.test(textoAtual)) {
            option.text = textoAtual.replace(/\(R\$\s*[^\)]+\)\s*$/, '(R$ ' + valorFormatado + ')');
        } else if (/R\$\s*[^\s]+\s*$/.test(textoAtual)) {
            option.text = textoAtual.replace(/R\$\s*[^\s]+\s*$/, 'R$ ' + valorFormatado);
        } else {
            option.text = textoAtual + ' (R$ ' + valorFormatado + ')';
        }
    }
    function atualizarBotaoFracionarMultas() {
        var idLancamento = document.getElementById('selCreditosProcesso').value;
        if (idLancamento && idLancamento !== 'null') {
            btn.style.display = '';
        } else {
    ...
    function abrirModalFracionarMultas() {

        if (document.getElementById('selCreditosProcesso').disabled) {
            alert('Não é possível fazer o fracionamento a partir de um lançamento que não está salvo.');
            return;
        }
      ...
      var url;
        ...
        topWin._mdLitFracionarMultasSalvo = false;
        
        $.ajax({
            url: '<?= SessaoSEI::getInstance()->assinarLink('controlador_ajax.php?acao_ajax=md_lit_obter_link_fracionar_multas_modal') ?>',
            type: 'POST',
            data: {
                'idLancamento': $('#hdnCreditosProcesso').val()
            },
            async: false,
            success: function (response) {
        ...
        infraAbrirJanelaModal(url, 1250, 470, false, function () {
            var topWin = window.top || window;
            if (topWin._mdLitFracionarMultasSalvo) {
                topWin._mdLitFracionarMultasSalvo = false;
                consultarExtratoMulta();
                $("#btnCancelar").click();
            }
