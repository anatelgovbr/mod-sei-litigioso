...
class MdLitSituacaoLancamIntINT extends InfraINT {
    ...
    public static function montarTabelaSituacaoLancamentoIntegracao(MdLitSituacaoLancamIntDTO $objMdLitSituacaoLancamIntDTO){
        ...
        // Caso a requisição nao tenha sucesso lança excecao
        if ($arrResultadoWebService['suc'] == false) {
            $msg = mb_convert_encoding($arrResultadoWebService['msg'], 'ISO-8859-1', 'UTF-8');
            $exception = new InfraException();
            $exception->adicionarValidacao(("Não foi possível carregar o web-service: {$objMdLitSituacaoLancamIntDTO->getStrEnderecoWsdl()}.
            {$msg}"));
