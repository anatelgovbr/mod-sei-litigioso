    ...
    public static function montarXMLDocumentoProcedimentoDetalhe($numeroSEI, $idTipoControleLitigioso, $tipo)
    {
        ...
        if ($tipo == 'd') {
            ...
            if (count($objProcedimentoDTO) > 0) {
                ...
                // OPÇÃO: Pode sobrestar a tramitaçãoo de outros processos
    ...
    public static function montarXmlInteressadoProcesso($idProcedimento, $idMdLitControle, $idMdLitTipoControle, $arrIdContato = array())
    {
        ...
        if ($arrObjContatoDTO) {
            ...
            foreach ($arrObjContatoDTO as $objContatoDTO) {
                ...
                $xml .= htmlentities(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_lit_dado_interessado_cadastrar&id_tp_controle=' . $idMdLitTipoControle . '&id_contato=' . $objContatoDTO->getNumIdContato() . '&id_procedimento=' . $idProcedimento . '&id_md_lit_controle=' . $idMdLitControle));
                $xml .= "</UrlDadosComplementares>\n";
