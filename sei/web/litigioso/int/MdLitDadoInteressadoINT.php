        ...
        foreach ($arrObjMdLitParamEntradaDTO as $objMdLitParamEntradaDTO){
            switch ($objMdLitParamEntradaDTO->getNumIdMdLitNomeFuncional()){
                ...
                case MdLitNomeFuncionalRN::$CNPJ_CPF:
                    ...
                    $strNumero = strtoupper(InfraUtil::retirarFormatacao($filtro['cpf_cnpj'], false));
                    if (trim($strNumero) == '') {
                        $filtroWS[$objMdLitParamEntradaDTO->getStrCampo()] = '';
                    }
                    if (strlen($strNumero) > 11 || preg_match('/[A-Z]/', $strNumero)) {
                        $filtroWS[$objMdLitParamEntradaDTO->getStrCampo()] = ctype_digit($strNumero) ? str_pad($strNumero, 14, '0', STR_PAD_LEFT) : $strNumero;
                    } else {
