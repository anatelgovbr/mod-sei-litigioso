...
$numPrazoDiasSugVenc = null;
$numPrazoDiasCritVencMin = null;
...
switch($_GET['acao']) {
    ...
    case 'md_lit_processo_situacao_cadastrar':
        ...
        if ($objMdLitTipoControleDTO) {
            $numPrazoDiasSugVenc = $objMdLitTipoControleDTO->getNumPrazoDiasSugVenc();
            $numPrazoDiasCritVencMin = $objMdLitTipoControleDTO->getNumPrazoDiasCritVencMin();
        }

        if (InfraString::isBolVazia($numPrazoDiasSugVenc) || InfraString::isBolVazia($numPrazoDiasCritVencMin)) {
            throw new InfraException('Prazos de Data de Vencimento não parametrizados no Tipo de Controle Litigioso.');
        }
...
//            }
        ...
        if($objMdLitLancamentoDTO){
            ...
            $dataVencimento = InfraData::calcularData($numPrazoDiasSugVenc, InfraData::$UNIDADE_DIAS, InfraData::$SENTIDO_ADIANTE, $dataDecisaoAplicacaoMulta);
        }
