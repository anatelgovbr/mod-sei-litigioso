...
class MdLitLancamentoDTO extends InfraDTO {
  ...
  public function montar() {
    ...
    $this->adicionarAtributoTabela(InfraDTO::$PREFIXO_NUM, 'IdMdLitLancamentoInicial', 'id_md_lit_lancamento_inicial');
    ...
    $this->configurarFK('IdMdLitLancamentoInicial', 'md_lit_lancamento', 'id_md_lit_lancamento');
    ...
    $this->configurarFK('IdProcedimento', 'protocolo', 'id_protocolo');
      ...
      $this->adicionarAtributoTabelaRelacionada(InfraDTO::$PREFIXO_DBL, 'CpfInteressado', 'cpf', 'md_lit_dado_interessado');
      $this->adicionarAtributoTabelaRelacionada(InfraDTO::$PREFIXO_STR, 'CnpjInteressado', 'cnpj', 'md_lit_dado_interessado');
  }

    public function getStrNomeTipoLancamentoComValor($index = null){
        $retorno = '';

        $nomeComplemento = $this->getComplementoNomeTipoLancamento();
        switch ($this->getStrTipoLancamento()){
            case MdLitLancamentoRN::$TIPO_LANCAMENTO_PRINCIPAL : $retorno = 'Principal'.$nomeComplemento.' (R$ '.InfraUtil::formatarDin(InfraUtil::prepararDbl($this->getDblVlrLancamento())).')';
                break;
            ...
            case MdLitLancamentoRN::$TIPO_LANCAMENTO_MAJORADO :
                $ordemMajorado = is_null($index) ? '' : ' ' . ($index);
                $retorno = 'Majorado'.$ordemMajorado.$nomeComplemento.' (R$ '.InfraUtil::formatarDin(InfraUtil::prepararDbl($this->getDblVlrLancamento())).')';
                break;
        }
        return $retorno;
    }

    private function getComplementoNomeTipoLancamento()
    {
        $retorno = '';
        $objMdLitLancamentoSecundarioDTO = $this->buscarLancamentoSecundario();
        
        // SE HOUVER LANCAMENTO LIGADO A ESTE, ENTГO ELE Й UM PRINCIAL
        if($objMdLitLancamentoSecundarioDTO){
            $retorno = ' inicial';
        }

        // SE ESSE LANCAMENTO FOR SECUNDARIO
        if($this->getNumIdMdLitLancamentoInicial()){
            $retorno = ' secundбrio';
        }
    ...
    private function buscarLancamentoSecundario()
    {
        $objMdLitLancamentoDTO = new MdLitLancamentoDTO;
        $objMdLitLancamentoDTO->setNumIdMdLitLancamentoInicial($this->getNumIdMdLitLancamento());
        $objMdLitLancamentoDTO->retNumIdMdLitLancamento();
        return (new MdLitLancamentoRN)->consultar($objMdLitLancamentoDTO);
