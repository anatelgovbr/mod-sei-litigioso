<?php
    ...
    class LitigiosoIntegracao extends SeiIntegracao
    {
        ...
        {
            return 'Anatel - Agência Nacional de Telecomunicaç?es';
        }
        ...
        public function processarControlador($strAcao)
        {
            switch ($strAcao) {
                ...
                case 'md_lit_dado_interessado_cadastrar':
                ...
                case 'md_lit_fracionar_multa_salvar':
                    require_once dirname(__FILE__).'/md_lit_fracionar_multas.php';
                    return true;
        ...
        public function processarControladorAjax($strAcaoAjax)
        {
            ...
            switch ($strAcaoAjax) {
                ...
                case 'md_lit_obter_link_fracionar_multas_modal':
                    $xml = MdLitLancamentoINT::obterLinkModalFracionarMultas($_POST['idLancamento']);
                    break;

                case 'md_lit_fracionar_multas_modal':
                    $xml = MdLitLancamentoINT::prepararFracionarMultas($_POST);
                    break;
        ...
        public function montarBotaoProcesso(ProcedimentoAPI $objProcedimentoAPI)
        {
             ...
             *  O ícone n?o deve mais ser apresentado, nem mesmo aos Controles já criados anteriormente."
             */
            ...
            //RN1 - bot?o "Cadastrar/Alterar Controle Litigioso" somente será apresentado caso o tipo de processo e unidade,
            ...
            if (count($ArrObjRelTipoControleLitigiosoTipoProcedimentoDTO) > 0 && count($ArrObjRelTipoControleLitigiosoUnidadeDTO) > 0 && $objProcedimentoAPI->getSinAberto() == 'S') {
                ...
                //aç?o se o processo n?o estiver aberto será somente de consulta
                $imgIcone             = "modulos/litigioso/imagens/svg/cadastro_ctrl_litigioso.svg?".Icone::VERSAO;
            ...
            //Add bot?o no processo quando já existe Situaç?o Cadastrada
        ...
        {
            /*
             * Estando em Processo já cadastrado em Controle Litigioso e em tipos de documentos associados a Situaç?es, aparece o bot?o "Controle Litigioso - Cadastro de Situaç?es".
             * */
            ...
            if ($existePermissao && $existePermiConsulta && $objProcedimentoAPI->getSinAberto() == 'S' && $objProcedimentoAPI->getCodigoAcesso() > 0) {
                ...
                if (!is_null($idTpControle)) {
                    ...
                    foreach ($arrObjDocumentoAPI as $objDocumentoAPI) {
                        //Busca todas as situaç?es dos tipos de controle que possuem esse processo vinculado
                        if ($countIsSituacao) {
                            ...
                            //Verifica se alguma das situaç?es possui esse documento vinculado
        ...
        public function montarIconeProcesso(ProcedimentoAPI $objProcedimentoAPI){
            ...
            if (SessaoSEI::getInstance()->verificarPermissao('md_lit_processo_situacao_consultar') && $objProcedimentoAPI->getCodigoAcesso() > 0 ) {
                ...
                //buscando a intimaç?o da instauraç?o( a intimaç?o da instauraç?o será sempre a primeira situaç?o de intimaç?o depois da instauraç?o)
                ...
                if($objMdLitProcessoSituacaoIntimacaoInstauracao){
                    ...
                    if($diferencaEntreDias > 1){
                        $diferencaEntreDias = '\nTempo desde a Intimaç?o: ' . $diferencaEntreDias . ' dias';
                    }else{
                        $diferencaEntreDias = '\nTempo desde a Intimaç?o: ' . $diferencaEntreDias . ' dia';
                    }
                }
                ...
                $titulo         .= 'Fase: '. $objMdLitProcessoSituacaoDTO->getStrNomeFase(). ' \nSituaç?o: '. $objMdLitProcessoSituacaoDTO->getStrNomeSituacao() . $tipoSituacao.' ' . $diferencaEntreDias;
        ...
        public function excluirDocumento(DocumentoAPI $objDocumentoAPI){
            ...
            if (SessaoSEI::getInstance()->verificarPermissao('md_lit_processo_situacao_consultar')) {
                ...
                if(!empty($arrMdLitProcessoSituacaoDTO)){
                    $msg = 'N?o é permitido Excluir este documento, pois ele está relacionado com as Fases/Situaç?es abaixo listadas no SEI Litigioso:\n \n';
        ...
        public function cancelarDocumento(DocumentoAPI $objDocumentoAPI){
            ...
            if (SessaoSEI::getInstance()->verificarPermissao('md_lit_processo_situacao_consultar')) {
                ...
                if(!empty($arrMdLitProcessoSituacaoDTO)){
                    $msg = 'N?o é permitido Cancelar este documento, pois ele está relacionado com as Fases/Situaç?es abaixo listadas no SEI Litigioso:\n \n';
...
;
            if (SessaoSEI::getInstance()->verificarPermissao('md_lit_dado_interessado_listar')) {
                ...
                if (count($arrObjMdLitDadoInteressadoDTO)) {
                    ...
                    $msg = 'N?o é permitido excluir este Contato, pois ele é utilizado pelo Módulo Litigioso nos seguintes processos:\n ';
        ...
        public function excluirTipoProcesso($arrObjTipoProcedimentoDTO)
        {
            ...
            if ($siglas) {
                $msg = 'N?o é possóvel excluir este Tipo de Processo, pois ele ainda está associado aos Tipos de Controle Litigioso abaixo:\n\n';
                $msg .= $siglas;
        ...
        public function desativarTipoProcesso($arrObjTipoProcedimentoDTO)
        {
            ...
            if ($siglas) {
                $msg = 'N?o é possível desativar este Tipo de Processo, pois ele ainda está associado aos Tipos de Controle Litigioso abaixo:\n\n';
                $msg .= $siglas;
                $objInfraException->lancarValidacao($msg);
            }
        }
        ...
        public function desativarUnidade($arrObjUnidadeAPI)
        {
            ...
            if (!empty($arrObjTipoControleLitigiosoUnidadeDTO)) {
                ...
                $objInfraException->lancarValidacao('Unidade n?o pode ser desativada, pois está sendo utilizada no Tipo de Controle Litigioso: '. $objTipoControleLitigiosoUnidadeDTO->getStrSiglaTipoControleLitigioso());
            }
        ...
        public function anexarProcesso(ProcedimentoAPI $objProcedimentoAPIPrincipal, ProcedimentoAPI $objProcedimentoAPIAnexado)
        {
            ...
            if($conclusao['emAndamento'] === true){
                $msg = 'N?o é possível Anexar o Processo indicado, pois ele ainda está em andamento no fluxo dos Tipos de Controle Litigioso abaixo:\n\n';
        ...
        protected function verificaModuloPeticionamentoInstalado($versao = null)
        {
            ...
            if (is_array($arrModulos) && array_key_exists('PeticionamentoIntegracao', $arrModulos)) {
                ...
                //vers?o do parametro é igual ou maior que a enviada
                if (!is_null($versao)) {
