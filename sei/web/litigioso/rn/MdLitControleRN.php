    ...
    class MdLitControleRN extends InfraRN
    {
        ...
        protected function listarInteressadoProcessoConectado($arrParametros)
        {
            ...
            if ($arrIdContato) {
                ...
                if($arrParametros['idMdLitControle']){
        ...
        protected function cadastrarCompletoControlado($paramsPost)
        {
            try {
                ...
                if (count($arrDI) == 0) {
                    $objInfraException = new InfraException();
                    $objInfraException->lancarValidacao('Informe ao menos um Dispositivo Normativo ou Conduta.');
                }
                ...
                if ($paramsPost['hdnTbInteressado'] != '') {
                    ...
                    foreach ($arrLinhasTbInteressado as $key => $linha) {
                        ...
                        if($objContatoDTO){
                            if($objContatoDTO->getStrStaNatureza() == ContatoRN::$TN_PESSOA_JURIDICA){
                                $mdLitDadoInteressadoDTO->setStrCnpj(strtoupper(InfraUtil::retirarFormatacao($objContatoDTO->getStrCnpj(), false)));
                            }else{
                    ...
                    $mdLitDadoInteressadoDTO = new MdLitDadoInteressadoDTO();
                    $mdLitDadoInteressadoDTO->setNumIdContato($arrIdContatoAtivo, InfraDTO::$OPER_NOT_IN);
                    $mdLitDadoInteressadoDTO->setNumIdMdLitControle($objControleLitigiosoDTO->getNumIdControleLitigioso());
                    $mdLitDadoInteressadoDTO->setDblIdProcedimentoMdLitTipoControle($paramsPost['idProcedimento']);
                    $mdLitDadoInteressadoDTO->retTodos();

                    $arrDadoInteressadoRemover = $objMdLitDadoInteressadoRN->listar($mdLitDadoInteressadoDTO);

                    $countDadoInteressadoRemover = is_array($arrDadoInteressadoRemover) ? count($arrDadoInteressadoRemover) : 0;

                    if($countDadoInteressadoRemover > 0) {
                        $objMdLitDadoInteressadoRN->removerInteressado($arrDadoInteressadoRemover);
                    }
                    //FIM INTERESSADOS;
                }
