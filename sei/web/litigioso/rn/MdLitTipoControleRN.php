    ...
    class MdLitTipoControleRN extends InfraRN
    {
        ...
        protected function cadastrarControlado(MdLitTipoControleDTO $objTipoControleLitigiosoDTO)
        {
            try {
                ...
                $this->validarNumPrazoDiasSugVenc($objTipoControleLitigiosoDTO, $objInfraException);
                $this->validarNumPrazoDiasCritVencMin($objTipoControleLitigiosoDTO, $objInfraException);
        ...
        private function validarNumPrazoDiasSugVenc(MdLitTipoControleDTO $objTipoControleLitigiosoDTO, InfraException $objInfraException)
        {
            if (InfraString::isBolVazia($objTipoControleLitigiosoDTO->getNumPrazoDiasSugVenc())) {
                $objInfraException->adicionarValidacao('Prazo em Dias de Sugestão de Data de Vencimento não informado.');
                return;
            }

            if (!is_numeric($objTipoControleLitigiosoDTO->getNumPrazoDiasSugVenc()) || intval($objTipoControleLitigiosoDTO->getNumPrazoDiasSugVenc()) != $objTipoControleLitigiosoDTO->getNumPrazoDiasSugVenc() || $objTipoControleLitigiosoDTO->getNumPrazoDiasSugVenc() < 0) {
                $objInfraException->adicionarValidacao('Prazo em Dias de Sugestão de Data de Vencimento deve ser um número inteiro maior ou igual a zero.');
            }
        }

        private function validarNumPrazoDiasCritVencMin(MdLitTipoControleDTO $objTipoControleLitigiosoDTO, InfraException $objInfraException)
        {
            if (InfraString::isBolVazia($objTipoControleLitigiosoDTO->getNumPrazoDiasCritVencMin())) {
                $objInfraException->adicionarValidacao('Prazo em Dias para Crítica de Data de Vencimento Mínima não informado.');
                return;
            }

            if (!is_numeric($objTipoControleLitigiosoDTO->getNumPrazoDiasCritVencMin()) || intval($objTipoControleLitigiosoDTO->getNumPrazoDiasCritVencMin()) != $objTipoControleLitigiosoDTO->getNumPrazoDiasCritVencMin() || $objTipoControleLitigiosoDTO->getNumPrazoDiasCritVencMin() < 0) {
                $objInfraException->adicionarValidacao('Prazo em Dias para Crítica de Data de Vencimento Mínima deve ser um número inteiro maior ou igual a zero.');
            }
        }
        ...
        protected function alterarControlado(MdLitTipoControleDTO $objTipoControleLitigiosoDTO)
        {
            ...
            try {
                ...
                if ($objTipoControleLitigiosoDTO->isSetNumPrazoDiasSugVenc()) {
                    $this->validarNumPrazoDiasSugVenc($objTipoControleLitigiosoDTO, $objInfraException);
                }

                if ($objTipoControleLitigiosoDTO->isSetNumPrazoDiasCritVencMin()) {
                    $this->validarNumPrazoDiasCritVencMin($objTipoControleLitigiosoDTO, $objInfraException);
                }
        ...
        protected function getObjTipoControlePorIdConectado($idTipoControle){
            try{
                ...
                $objMdLitTipoControleDTO->retNumPrazoDiasCritVencMin();
