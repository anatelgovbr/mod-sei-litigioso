...
//        'A situação <Fase / Situação> selecionada acima estará associada à Situação <Fase / Situação: última situação obrigatória adicionada na grid>, já adicionada na lista abaixo, para qual a Situação esperada seria <Fase / Situação: situação esperada, de acordo com a parametrização>. Favor selecionar a Fase e Situação correta, de acordo com a ordem da parametrização do Tipo de Controle Litigioso acima.';
    ...
    protected function processarCadastrarControlado($post)
    {
        try {
            ...
            //SALVAR E ATUALIZAR SITUAÇÕES
            $this->cadastrarSituacoesProcesso($post);

            $objMdLitLancamentoRN = new MdLitLancamentoRN();
            $arrLancamentosDecisoesAlteradas = array();
            if ($post['hdnIdMdLitFuncionalidade'] == MdLitIntegracaoRN::$ARRECADACAO_RETIFICAR_LANCAMENTO) {
                $arrLancamentosDecisoesAlteradas = $objMdLitLancamentoRN->identificarLancamentosDecisoesAlteradas($post);
            }

            //SALVAR OU ATUALIZAR DECISÕES
            $arrIdDecisao = (new MdLitDecisaoRN)->prepararCadastrarDecisoes($post);

            if (!empty($arrLancamentosDecisoesAlteradas)) {
                $objMdLitLancamentoRN->processarLancamentosDecisoesAlteradas($arrLancamentosDecisoesAlteradas, $arrIdDecisao, $post);
                return;
            }

            //SALVAR OU ATUALIZAR LANCAMENTO
            $objMdLitLancamentoDTO = $objMdLitLancamentoRN->salvarLancamento($post);

            //VINCULAR LANCAMENTO ÀS DECISOES
            (new MdLitRelDecisLancamentRN())->vincularDecisaoComLancamento($arrIdDecisao, $objMdLitLancamentoDTO);

            //ATUALIZAR VALORES DE LANCAMENTO INICIAL E SECUNDARIO QUANDO FOR FRACIONADO E HOUVER ALTERAÇÃO DE VALOR EM AMBOS
            if($post['hdnLancamentoFracionado'] == 'true')
                $objMdLitLancamentoDTO = $objMdLitLancamentoRN->atualizarDadosLancamentoInicialSecundario($objMdLitLancamentoDTO, $post);

            //ENVIA PARA FINANCEIRO
            $objMdLitLancamentoRN->enviarFinanceiro($post, $objMdLitLancamentoDTO);
    ...
    private function cadastrarSituacoesProcesso($post)
    {
        $arrSituacao = PaginaSEI::getInstance()->getArrItensTabelaDinamica($post['hdnTbSituacoes']);

        $this->controlarExclusaoSituacoes($post['hdnIdProcedimento'], $arrSituacao, $post);
    ...
    private function _realizaAlteracaoSituacao($arrDados)
    {
        ...
        // CASO A SITUAÇÃO ESTEJA RELACIONADA A ALGUMA DATA EM ALGUM LANCAMENTO DEVE ATUALZIAR ESSA DATA
        // PODE SER DATA DE: INTIMACAO / DECISAO / RECURSO / DECISAO DEFINITIVA
        (new MdLitLancamentoRN)->atualizarDataDeSituacoesRelacionadaLancamento($objMdLitProcessoSitDTO);
    ...
    private function controlarExclusaoSituacoes($idProcedimento, $arrSituacao, $post)
    {
        $idsExcluidos = array();
    ...
    //todo remover essa função e colocar no lugar correto
        ...
        foreach ($idsExcluidos as $idExcluido) {
            ...
            if($objMdLitProceSit) {
                ...
                foreach ($arrObjMdLitLancamento as $objMdLitLancamento) {
                    //SE O LANCAMENTO JA ESTIVER COM VALOR ZERADO/NEGATIVO (POR EXEMPLO, DECISOES
                    //DESSE BOLETO JA FORAM REDUZIDAS A ZERO EM UMA ETAPA ANTERIOR DO DESFAZER
                    //FASE POR FASE), CANCELAR EM VEZ DE RETIFICAR - RETIFICAR COM VALOR ZERADO
                    //E REJEITADO PELO SIGEC (ERRO SOAP 90004 "VALOR INFORMADO E INVALIDO").
                    if ((float) $objMdLitLancamento->getDblVlrLancamento() <= 0) {
                        $objMdLitLancamentoRN->cancelarLancamentoPorValorZerado(
                            $objMdLitLancamento,
                            'Cancelamento da situacao de TRANSITO EM JULGADO',
                            'Multa cancelada por cancelamento da situacao de TRANSITO EM JULGADO.'
                        );
                        continue;
                    }
    ...
    //todo organizar trecho no local apropriado
    ...
    public function retornarUltimaSituacaoCadastradaProcesso($idProcedimento)
    {
        $objMdLitProcessoSituacaoDTO = new MdLitProcessoSituacaoDTO();
        $objMdLitProcessoSituacaoDTO->retNumIdMdLitProcessoSituacao();
    ...
    //TODO deve atualizar a data na qual esteja realacionada
    ...
    //         $objMdLitLancamento->setDtaApresentacaoRecurso($hdnDtApresentacaoRecurso);
    //         $objMdLitLancamentoRN->alterar($objMdLitLancamento);
    //     }
    // }
