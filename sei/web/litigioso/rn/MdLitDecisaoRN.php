...
class MdLitDecisaoRN extends InfraRN
{
    ...
    protected function cadastrarControlado($arrDecisao)
    {
        try {
            ...
            foreach ($arrDecisao['lista'] as $decisao) {
                ...
                if($decisao[20] != 'undefined')
                    $objMdLitDecisaoDTO->setNumIdMdLitDecisaoOrigem($decisao[20]);
                ...
                if (preg_match('/^novo_/', $decisao[0])) {
                    ...
                    //A DECISAO PODE TER SIDO EXCLUIDA NA MESMA OPERACAO (POR EXEMPLO, AO
                    //EXCLUIR A SITUACAO A QUE ELA PERTENCIA VIA
                    //MdLitProcessoSituacaoRN::excluirControlado(), QUE APAGA AS DECISOES
                    //VINCULADAS ANTES DESTE PONTO). A TABELA hdnTbDecisao ENVIADA PELO POST
                    //FOI MONTADA NO CARREGAMENTO DA PAGINA E PODE AINDA REFERENCIAR ESSA
                    //DECISAO COMO EXISTENTE; SE ELA JA NAO EXISTE MAIS, NAO HA O QUE ALTERAR.
                    $objMdLitDecisaoExistenteDTO = new MdLitDecisaoDTO();
                    $objMdLitDecisaoExistenteDTO->setNumIdMdLitDecisao($decisao[0]);
                    $objMdLitDecisaoExistenteDTO->setNumMaxRegistrosRetorno(1);
                    $objMdLitDecisaoExistenteDTO->retNumIdMdLitDecisao();
                    if (!$this->consultar($objMdLitDecisaoExistenteDTO)) {
                        continue;
                    }
        ...
        $mapLancamentoPorDecisao = array();
        if (count($arrObjMdLitDecisaoDTO)) {
            $arrIdMdLitDecisao = InfraArray::converterArrInfraDTO($arrObjMdLitDecisaoDTO, 'IdMdLitDecisao');
            ...
            $objMdLitRelDecisLancamentDTO->setNumIdMdLitDecisao($arrIdMdLitDecisao, InfraDTO::$OPER_IN);
            $arrRelDecisLanc = (new MdLitRelDecisLancamentRN())->listar($objMdLitRelDecisLancamentDTO);
            foreach ($arrRelDecisLanc as $objRelDecisLanc) {
                $mapLancamentoPorDecisao[$objRelDecisLanc->getNumIdMdLitDecisao()] = $objRelDecisLanc->getNumIdMdLitLancamento();
            }
        }
                if (count($arrObjMdLitDecisaoDTO)) {
    ...
    protected function retornaDadosDecisaoCadastradasConectado($idProcedimento)
    {
        ...
        foreach ($arrObjMdLitDecisaoDTO as $key => $objMdLitDecisaoDTO) {
            ...
            $idDecisao = $objMdLitDecisaoDTO->getNumIdMdLitDecisao();
            $infracaoArr[$key][] = array_key_exists($idDecisao, $mapLancamentoPorDecisao) ? $mapLancamentoPorDecisao[$idDecisao] : 'null';
        }
...
//            echo $ret; exit;
    ...
    public function prepararCadastrarDecisoes($post)
    {
        ...
        return $this->cadastrar($arrDecisao);
    }
