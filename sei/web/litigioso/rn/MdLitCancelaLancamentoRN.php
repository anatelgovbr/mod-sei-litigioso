...
class MdLitCancelaLancamentoRN extends InfraRN {
  ...
  public function montarMotivoCancelamento(){
    ...
    public function cancelarLancamento($objMdLitLancamentoDTO, $post)
    {
        try {

            //BUSCA INTEGRACAO E VALIDA A CONFIGURACAO ANTES DE CANCELAR NO FINANCEIRO
            $objMdLitIntegracaoConsultarLancamento = $this->retornaIntegracaoConsultarLancamento();
          
            //BUSCA OBJETO DE INTEGRACAO PARA CANCELAR LANCAMENTO
            $objMdLitIntegracaoDTO    = (new MdLitIntegracaoRN())->retornarObjIntegracaoDTOPorFuncionalidade($post['hdnIdMdLitFuncionalidade']);
            ...
            //MONTA OS DADOS NECESSARIO PARA ENVIAR PARA O FINANCEIRO SERVICE PARA CANCELAR O LANCAMENTO
            $montarParametroEntrada = $this->_montarParametroEntradaCancelamentoCredito($objMdLitIntegracaoDTO, $objMdLitLancamentoDTO, $post);

            //ENVIA O PEDIDO DE CANCELAMENTO PARA FINANCEIRO          
            $arrRetorno = $objMdLitSoapClienteRN->enviarDadosSigecLancamento($objMdLitIntegracaoDTO, $montarParametroEntrada, MdLitMapearParamEntradaRN::$PARAM_PRINCIPAL_SIGEC_CANC_LANCAMENTO);

            //FAZ UMA CONSULTA NO FINANCEIRO PARA ATUALIZAR O LANCAMENTO DO LADO DO SEI
            $objMdLitConsultarLancRN  = new MdLitConsultarLancamentoRN();
            $objMdLitConsultarLancRN->consultarLancamento($objMdLitLancamentoDTO);

            if ($arrRetorno) {
                //FAZ O REGISTRO NA TABELA DE CANCELA LANCAMENTO
                $this->_prepararObjCancelaLancamento($objMdLitLancamentoDTO->getNumIdMdLitLancamento(), $post);

                //INCLUI NO HITORICO CANCELAMENTO DE LANCAMENTO
                (new MdLitHistoricLancamentoRN())->incluirHistoricoCancelaLancamento($objMdLitLancamentoDTO);
            }
    ...
    private function retornaIntegracaoConsultarLancamento()
    {
        $objMdLitIntegracaoRN     = new MdLitIntegracaoRN();
        $objMdLitIntegracaoConsultarLancamento = $objMdLitIntegracaoRN->retornarObjIntegracaoDTOPorFuncionalidade(MdLitIntegracaoRN::$ARRECADACAO_CONSULTAR_LANCAMENTO);

        if(is_null($objMdLitIntegracaoConsultarLancamento))
            throw new InfraException('É necessário realizar a integração com a funcionalidade de Consultar Lançamento.');
        
        if(empty($objMdLitIntegracaoConsultarLancamento->getArrObjMdLitMapearParamSaidaDTO()) && empty($objMdLitIntegracaoConsultarLancamento->getArrObjMdLitMapearParamEntradaDTO()))
            throw new InfraException('Os parâmetros de entrada e saída não foram parametrizados. Contate o Gestor do Controle.');
        
        return $objMdLitIntegracaoConsultarLancamento;
    }

    private function _montarParametroEntradaCancelamentoCredito($objMdLitIntegracaoDTO, $objMdLitLancamentoDTO, $post)
    {
        ...
        foreach ($objMdLitIntegracaoDTO->getArrObjMdLitMapearParamEntradaDTO() as $objMdLitMapearParamEntradaDTO) {
            ...
            switch ($idIntegracao) {
                ...
                case MdLitMapearParamEntradaRN::$ID_PARAM_CANCELAR_LANCAMENTO['NUM_PROCESSO']:
                    $numProcesso = $objMdLitLancamentoRN->retornaNumProcessoFormatado($objMdLitLancamentoDTO->getDblIdProcedimento());
    ...
    private function realizarValidacoes($objMdLitIntegracaoConsultarLancamento)
    {
      if(is_null($objMdLitIntegracaoConsultarLancamento)){
            throw new InfraException('É necessário realizar a integração com a funcionalidade de Consultar Lançamento.');
        }
    ...
    // public function cancelarLancamento($post){
    //     $objInfraException        = new InfraException();
    ...
    private function _prepararObjCancelaLancamento($idLancamento, $post){
    ...
    //             case MdLitMapearParamEntradaRN::$ID_PARAM_CANCELAR_LANCAMENTO['NUM_PROCESSO']:
    //                 $numProcesso = $objMdLitLancamentoRN->retornaNumProcessoFormatado($post['hdnIdProcedimento']);
    //                 $montarParametroEntrada[$objMdLitMapearParamEntradaDTO->getStrCampo()] = $numProcesso;
    //                 break;
